...
from moviepy.editor import VideoFileClip, AudioFileClip # Adicionado AudioFileClip do moviepy
...
def converter_midia_gui(caminho_entrada, caminho_saida, formato_saida, progress_callback):
    ...
    is_video_input = False # Determina se o arquivo de entrada é um vídeo
    clip = None
    try:
        ...
        try:
            # Se não for vídeo, tenta como áudio
            clip = AudioSegment.from_file(caminho_entrada)
            is_video_input = False
            progress_callback(f"Detectado áudio de entrada. Convertendo para .{formato_saida}...", "info")
        except Exception as e_audio:
            progress_callback(f"Erro ao carregar mídia: {e_video} / {e_audio}. Formato não reconhecido ou arquivo corrompido.", "error", True)
            return False

    try:
        if is_video_input:
            # Para vídeo, MoviePy lida com isso. Codec padrão para MP4/MKV é libx264, para áudio aac
            clip.write_videofile(caminho_saida, codec="libx264", audio_codec="aac", logger=None)
        else:
            # Para áudio, pydub ou moviepy.AudioFileClip. MoviePy AudioFileClip é mais robusto para áudio a áudio.
            audio_clip_mp = AudioFileClip(caminho_entrada) # Usar AudioFileClip do moviepy para uniformidade
            audio_clip_mp.write_audiofile(caminho_saida, logger=None)
...
class ConvertTab(ctk.CTkFrame):
    def __init__(self, master, config_ref):
        ...
        self.is_video_input = False # Para saber se o input é vídeo ou áudio
    ...
    def iniciar_conversao(self):
        ...
        threading.Thread(target=self._executar_conversao_thread, args=(input_path, caminho_saida, selected_format)).start()

    def _executar_conversao_thread(self, input_path, output_path, format_):
        try:
            sucesso = converter_midia_gui(input_path, output_path, format_, self.update_status)
            if sucesso:
            ...
            self.master.after(0, self.btn_converter.configure, {'state': "normal"})
...
if __name__ == "__main__":
    ...
    try:
        from moviepy.editor import VideoFileClip, AudioFileClip # Adicionei AudioFileClip aqui
