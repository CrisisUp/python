...
def aplicar_estilo(tag, **kwargs):
    try:
        if campo_texto.tag_ranges(tk.SEL):
            ...
            messagebox.showwarning("Aviso", "Selecione um texto para aplicar o estilo.")
    except tk.TclError:
...
def alterar_cor_fonte():
    cor = colorchooser.askcolor(title="Escolha uma cor para o texto")
    if cor[1]:  # cor[1] é o código hexadecimal
        aplicar_estilo(f"cor_{cor[1]}", foreground=cor[1])
...
editar_menu.add_command(label="Alterar Cor da Fonte", command=alterar_cor_fonte)
